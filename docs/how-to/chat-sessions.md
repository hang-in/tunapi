...
In group chats, Tunapi stores a session per sender, so different people can work independently in the same chat.

## Working directory changes

When `session_mode = "chat"` is enabled, Tunapi clears stored chat sessions on startup if the current working directory differs from the one recorded in `telegram_chat_sessions_state.json`. This avoids resuming directory-bound sessions from a different project.
