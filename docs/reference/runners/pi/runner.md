Below is a concrete implementation spec for the **Pi (pi-coding-agent CLI)** runner shipped in Tunapi (v0.5.0).
...
Provide the **`pi`** engine backend so Tunapi can:
...
* Support resumable sessions via **`--session <token>`** (Tunapi emits a canonical resume line the user can reply with).
...
* `pi --resume/-r` opens an interactive session picker, so Tunapi uses `--session <token>` instead.
...
Pi does not accept `-- <prompt>` to protect prompts starting with `-`. Tunapi prefixes a leading space if the prompt begins with `-` so it is not parsed as a flag.
...
=== "tunapi config"
    ...
    ```sh
    tunapi config set default_engine "pi"
...
* `extra_args` lets you pass new Pi flags without changing Tunapi.
...
### 1) New file: `src/tunapi/runners/pi.py`
...
set up credentials before using Tunapi.

---

## Known pitfalls

* `--resume` is interactive; Tunapi uses `--session <path>` instead.
* Prompts that start with `-` are interpreted as flags by the CLI. Tunapi
  prefixes a space to make them safe.

---

If you want, I can also add a sample `tunapi.toml` snippet to the README or
