...
This page is a high-level map of Tunapi’s internal modules: what they do and how they fit together.
...
| `events.py` | Event factory helpers for building Tunapi events consistently. |

## Rendering and progress

| Module | Responsibility |
|--------|----------------|
| `progress.py` | Progress tracking: reduces tunapi events into progress snapshots. |
...
| `api.py` | Public plugin API boundary (`tunapi.api` re-exports). |
...
| Module | Responsibility |
|--------|----------------|
| `settings.py` | Loads `tunapi.toml` (TOML + env), validates with pydantic-settings. |
