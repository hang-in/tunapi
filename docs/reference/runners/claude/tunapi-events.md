# Claude Code -> Tunapi event mapping (spec)

This document describes how the Claude Code runner translates Claude CLI JSONL events into Tunapi events.

> **Authoritative source:** The schema definitions are in `src/tunapi/schemas/claude.py` and the translation logic is in `src/tunapi/runners/claude.py`. When in doubt, refer to the code.

The goal is to make a Claude runner feel identical to the Codex runner from the bridge/renderer point of view while preserving Tunapi invariants (stable action ids, per-session serialization, single completed event).
...
Tunapi requires **serialization per session id**:
...
This matches the Codex runner behavior in `tunapi/runners/codex.py`.

---

## 4. Event translation (Claude JSONL -> Tunapi)
...
**Mapping:**
- Emit a Tunapi `started` event as soon as `session_id` is known.
...
**Mapping:** emit a single Tunapi `completed` event:
...
Tunapi `completed.usage` should mirror the Claude `result.usage` object
...
Claude runner implementation summary (no Tunapi domain model changes):

1. [x] Create `tunapi/runners/claude.py` implementing `Runner` and (custom)
   resume parsing.
2. [x] Define `BACKEND` in `tunapi/runners/claude.py`:
...
## 8. Suggested Tunapi config keys
...
=== "tunapi config"
    ...
    ```sh
    tunapi config set claude.model "sonnet"
    tunapi config set claude.allowed_tools '["Bash", "Read", "Edit", "Write", "WebSearch"]'
...
Tunapi only maps these keys to Claude CLI flags; other options should be configured in Claude Code settings.
If `allowed_tools` is omitted, Tunapi defaults to `["Bash", "Read", "Edit", "Write"]`.
When `use_api_billing` is false (default), Tunapi strips `ANTHROPIC_API_KEY` from the Claude subprocess environment to prefer subscription billing.
