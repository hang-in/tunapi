...
So far, Tunapi runs in whatever directory you started it. If you want to work on a different repo, you have to:

1. Stop Tunapi
2. `cd` to the other repo
3. Restart Tunapi

Projects fix this. Once you register a repo, you can target it from chat—even while Tunapi is running elsewhere.

## 1. Register a project

Navigate to the repo and run `tunapi init`:
...
```
saved project 'happy-gadgets' to ~/.tunapi/tunapi.toml
```

This adds an entry to your config (Tunapi also fills in defaults like `worktrees_dir`, `default_engine`, and sometimes `worktree_base`):
...
Now you can start Tunapi from another repo. If you don't specify a project, Tunapi runs in the directory where you launched it.
...
Tunapi runs the agent in `~/dev/happy-gadgets`, not your current directory.
...
That `ctx:` line tells you which project is active. When you reply, Tunapi automatically uses the same project—you don't need to repeat `/happy-gadgets`.

## 3. Set up worktrees

Worktrees let you run tasks on feature branches without touching your main checkout. Instead of `git checkout`, Tunapi creates a separate directory for each branch.
...
=== "tunapi config"
    ...
    tunapi config set projects.happy-gadgets.worktree_base "main"
    ```
...
All from the same Telegram chat, without restarting Tunapi or changing directories.
...
| `worktree_base` | `null` | Base branch for new worktrees. If unset, Tunapi uses `origin/HEAD`, the current branch, or `master`/`main` (in that order). |
...
Run `tunapi init <alias>` in the repo first.
...
If the branch already exists locally, Tunapi uses it. For a fresh start, delete the worktree **and** the branch, or pick a new branch name.
