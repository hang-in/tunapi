---
name: tunapi-release
description: Prepare and ship a Tunapi release. Use when asked to cut a release, bump release versions, update changelog/spec/readme, tag v<major.minor.patch>, or trigger the GitHub release workflow.
---
...
Prepare a tagged release that matches the GitHub Actions release workflow. The workflow requires the tag version to match both `pyproject.toml` and `src/tunapi/__init__.py`.
...
- `src/tunapi/__init__.py`: `__version__ = "<major.minor.patch>"`
...
- Header: `# Tunapi Specification v<major.minor.patch> [YYYY-MM-DD]`
- Replace `Tunapi v<old>` and `Out of scope for v<old>` lines with the new version.
...
- The release workflow checks that the tag matches `pyproject.toml` and `src/tunapi/__init__.py`.
