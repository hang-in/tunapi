...
from ..settings import TunapiSettings, validate_settings_data
...
def _fail_missing_config(path: Path) -> None:
    ...
    if path.exists():
        typer.echo(f"error: invalid tunapi config at {display}", err=True)
    else:
        typer.echo(f"error: missing tunapi config at {display}", err=True)
...
def _normalized_value_from_settings(
    settings: TunapiSettings, segments: list[str]
) -> Any:
    node: Any = settings
    for segment in segments:
