# OpenCode to Tunapi Event Mapping

This document describes how OpenCode JSON events are translated to Tunapi's normalized event model.

> **Authoritative source:** The schema definitions are in `src/tunapi/schemas/opencode.py` and the translation logic is in `src/tunapi/runners/opencode.py`. When in doubt, refer to the code.
...
Tunapi:   StartedEvent(engine="opencode", resume=ResumeToken(engine="opencode", value="ses_XXX"))
```
...
Tunapi:   ActionEvent(engine="opencode", action=Action(kind="command"), phase="started")
```

**Completed phase** (when tool finishes):
```
OpenCode: {"type":"tool_use","part":{"tool":"bash","state":{"status":"completed","metadata":{"exit":0}}}}
Tunapi:   ActionEvent(engine="opencode", action=Action(kind="command"), phase="completed", ok=True)
```
...
Tunapi:   CompletedEvent(engine="opencode", ok=True, answer="<accumulated text>", usage={...})
```

If `step_finish` omits `reason`, Tunapi treats a clean process exit as successful completion and emits `CompletedEvent(ok=True)` with the accumulated usage.

**Error**:
```
OpenCode: {"type":"error","error":{"name":"APIError","data":{"message":"API rate limit exceeded"}}}
Tunapi:   CompletedEvent(engine="opencode", ok=False, error="API rate limit exceeded")
```

## Tool Kind Mapping

| OpenCode Tool | Tunapi ActionKind |
|---------------|-------------------|
| `bash`, `shell` | `command` |
