...
def test_duplicate_entrypoints_are_rejected(monkeypatch) -> None:
    entrypoints = [
        FakeEntryPoint(
            "dup",
            "tunapi.runners.one:BACKEND",
            plugins.ENGINE_GROUP,
            dist_name="one",
        ),
        FakeEntryPoint(
            "dup",
            "tunapi.runners.two:BACKEND",
            plugins.ENGINE_GROUP,
            dist_name="two",
        ),
    ]
...
def test_allowlist_filters_by_distribution(monkeypatch) -> None:
    entrypoints = [
        ...
        FakeEntryPoint(
            "thirdparty",
            "tunapi_thirdparty.backend:BACKEND",
            plugins.ENGINE_GROUP,
            dist_name="tunapi-thirdparty",
        ),
    ]
    install_entrypoints(monkeypatch, entrypoints)

    ids = plugins.list_ids(plugins.ENGINE_GROUP, allowlist=["tunapi"])
    assert ids == ["codex"]
...
def test_allowlist_canonicalizes_distribution_names(monkeypatch) -> None:
    entrypoints = [
        FakeEntryPoint(
            "slack",
            "tunapi.transport.slack:BACKEND",
            plugins.TRANSPORT_GROUP,
            dist_name="tunapi-transport-slack",
        )
    ]
    install_entrypoints(monkeypatch, entrypoints)

    ids = plugins.list_ids(
        plugins.TRANSPORT_GROUP, allowlist=["tunapi_transport.slack"]
    )
    assert ids == ["slack"]
...
def test_clear_load_errors_filters(monkeypatch) -> None:
    ...
    entrypoints = [
        ...
        FakeEntryPoint(
            "broken_transport",
            "tunapi.transports.broken:BACKEND",
            plugins.TRANSPORT_GROUP,
            loader=loader,
