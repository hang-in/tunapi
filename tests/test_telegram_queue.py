...
from tunapi.telegram.client import BotClient, TelegramClient, TelegramRetryAfter
...
async def test_edit_forum_topic_uses_outbox() -> None:
    ...
    result = await client.edit_forum_topic(
        chat_id=7, message_thread_id=42, name="tunapi @main"
    )

    assert result is True
    assert bot.calls == ["edit_forum_topic"]
    assert bot.topic_calls == [(7, 42, "tunapi @main")]
