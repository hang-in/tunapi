...
| OpenCode Event | Tunapi Event | Condition |
|----------------|--------------|-----------|
...
If `step_finish` omits `reason`, Tunapi treats a clean process exit as successful completion and emits `CompletedEvent(ok=True)` with accumulated usage.
