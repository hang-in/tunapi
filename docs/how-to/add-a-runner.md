...
This guide explains how to add a **new engine runner** to Tunapi.

A *runner* is the adapter between an engine-specific CLI (Codex, Claude Code, …) and Tunapi’s
...
Tunapi is designed so that adding a runner usually means **adding one new module** under
`src/tunapi/runners/` plus a small **msgspec schema** module under `src/tunapi/schemas/`—
no changes to the bridge, renderer, or CLI.

When writing code intended for plugins, prefer importing from `tunapi.api`
...
- Run `tunapi acme` (CLI subcommand is auto-registered).
...
### 1) Tunapi owns the domain model

Tunapi’s core types live in `tunapi.model`:
...
Tunapi deliberately treats the runner as the authority for:
...
This matters because Tunapi’s Telegram truncation logic preserves resume lines.
...
- The config table name (`[acme]` in `tunapi.toml`)
- The CLI subcommand (`tunapi acme`)
...
### Step 2 — Create `src/tunapi/schemas/acme.py` + `src/tunapi/runners/acme.py`
...
Tunapi discovers engines via **entrypoints**. Every engine backend must be exposed
as an entrypoint under `tunapi.engine_backends`, and the entrypoint name must match
the backend id.
...
acme = "tunapi.runners.acme:BACKEND"
```
...
### Step 3 — Translate Acme JSONL into Tunapi events

Most CLIs we integrate are JSONL-streaming processes.

Tunapi provides `JsonlSubprocessRunner`, which:

- spawns the CLI
- drains stderr and logs it
- reads stdout line-by-line as JSONL bytes
- calls your `decode_jsonl(...)` and then `translate(...)` to convert each event into Tunapi events
...
- `factory`: `EventFactory` instance for creating Tunapi events and tracking resume
...
Create a small schema module under `src/tunapi/schemas/` and expose a `decode_event(...)`
    ...
    *,
    ...
    match event:
        ...
        case acme_schema.ToolUse(id=tool_id, name=name, input=tool_input):
            ...
            # (Claude uses tunapi.utils.paths.relativize_command / relativize_path)
...
- When `resume=None`, Tunapi will acquire a per-session lock after it sees the first
...
Tunapi needs a way to build your runner from config.
...
Because engine backends are auto-discovered (`tunapi.engines`), you do **not** need
...
`tunapi.schemas.<engine>.decode_event`.
...
- **StartedEvent arrives too late**
  - If you wait until the end to emit `StartedEvent`, Tunapi can’t acquire the per-session lock
...
- **Multiple completion events**
  - Some CLIs emit multiple “final-ish” events. Decide which one becomes Tunapi’s `CompletedEvent`.
...
- **Resume line gets truncated**
  - Ensure `is_resume_line()` matches your `format_resume()` output. Tunapi tries to preserve
...
- [ ] `tunapi acme` appears automatically (module exports `BACKEND`).
