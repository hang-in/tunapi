...
from tunapi.worktrees import WorktreeError, ensure_worktree, resolve_run_cwd
...
def test_resolve_run_cwd_uses_root_when_branch_matches(
    ...
    monkeypatch.setattr("tunapi.worktrees.git_stdout", _fake_stdout)
    monkeypatch.setattr(
        "tunapi.worktrees.ensure_worktree",
        _unexpected,
    )
...
def test_ensure_worktree_creates_from_base(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tunapi.worktrees.git_ok", lambda *args, **kwargs: False)
    monkeypatch.setattr("tunapi.worktrees.resolve_default_base", lambda *_: "main")

    def _fake_git_run(args, cwd):
        calls.append(list(args))
        return SimpleNamespace(returncode=0, stdout="", stderr="")

    monkeypatch.setattr("tunapi.worktrees.git_run", _fake_git_run)
