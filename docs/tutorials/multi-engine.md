...
When you reply, Tunapi sees `claude --resume` and automatically uses Claude—you don't need to repeat `/claude`.
...
When Tunapi picks an engine, it checks (highest to lowest):
...
6. **Global default** — `default_engine` at the top of `tunapi.toml`
...
=== "tunapi config"
    ...
    tunapi config set projects.backend.path "~/dev/backend"
    tunapi config set projects.backend.default_engine "claude"
    ```
