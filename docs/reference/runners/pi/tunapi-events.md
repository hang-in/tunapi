# Pi -> Tunapi event mapping (spec)

This document describes how the Pi runner translates Pi CLI `--mode json` JSONL events into Tunapi events.

> **Authoritative source:** The schema definitions are in `src/tunapi/schemas/pi.py` and the translation logic is in `src/tunapi/runners/pi.py`. When in doubt, refer to the code.

The goal is to make Pi feel identical to the Codex/Claude runners from the bridge/renderer point of view while preserving Tunapi invariants (stable action ids, per-session serialization, single completed event).
...
- Pi does not support `-- <prompt>`; prompts starting with `-` must be
  prefixed (Tunapi does this automatically).
...
- `--resume/-r` opens an interactive session picker; it does not accept a
  session token. Tunapi must use `--session <token>` instead.

---

## 3. Session lifecycle + serialization

Tunapi requires **serialization per session token**:

- For new runs (`resume=None`), do **not** acquire a lock until a `started`
  event is emitted (Tunapi emits this as soon as the session header or first
  JSON event arrives).
...
## 4. Event translation (Pi JSONL -> Tunapi)

Pi emits `AgentSessionEvent` objects. Only a subset is required for Tunapi.
...
Tunapi `completed.usage` should mirror Pi's assistant `usage` object without
transformation.

---

## 7. Suggested Tunapi config keys
