...
def test_plugins_cmd_loads_and_reports_errors(monkeypatch) -> None:
    entrypoints = {
        ENGINE_GROUP: [
            FakeEntryPoint(
                "codex",
                "tunapi.runners.codex:BACKEND",
                ENGINE_GROUP,
                dist_name="tunapi",
            ),
            FakeEntryPoint(
                "broken",
                "tunapi.runners.broken:BACKEND",
                ENGINE_GROUP,
                dist_name="tunapi",
            ),
        ],
        TRANSPORT_GROUP: [
            FakeEntryPoint(
                "telegram",
                "tunapi.transports.telegram:BACKEND",
                TRANSPORT_GROUP,
                dist_name="tunapi",
            )
        ],
    ...
    monkeypatch.setattr(cli, "resolve_plugins_allowlist", lambda _settings: ["tunapi"])
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            ...
            PluginLoadError(
                TRANSPORT_GROUP,
                "wire",
                "tunapi.transports.wire:BACKEND",
                "tunapi",
                "missing",
            ),
            PluginLoadError(
    ...
    assert "codex (tunapi) enabled" in result.output
    assert "hello (thirdparty) disabled" in result.output
    assert "errors:" in result.output
    assert "engine broken (tunapi): boom" in result.output
    assert "transport wire (tunapi): missing" in result.output
...
def test_config_path_cmd_defaults_to_home(monkeypatch, tmp_path: Path) -> None:
    ...
    config_path = tmp_path / ".tunapi" / "tunapi.toml"
    ...
    assert result.exit_code == 0
    assert result.output.strip() == "~/.tunapi/tunapi.toml"
...
def test_doctor_rejects_non_telegram_transport(monkeypatch) -> None:
    ...
    assert result.exit_code == 1
    assert "transports only" in result.output
