...
from tunapi.utils.git import git_is_worktree, git_ok, git_run, git_stdout
from tunapi.utils.git import resolve_default_base, resolve_main_worktree_root


def test_resolve_main_worktree_root_returns_none_when_no_git(monkeypatch) -> None:
    monkeypatch.setattr("tunapi.utils.git.git_stdout", lambda *args, **kwargs: None)
...
def test_resolve_main_worktree_root_prefers_common_dir_parent(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tunapi.utils.git.git_stdout", _fake_stdout)
...
def test_resolve_default_base_prefers_master_over_main(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tunapi.utils.git.git_stdout", _fake_stdout)
    monkeypatch.setattr("tunapi.utils.git.git_ok", _fake_ok)
...
def test_git_run_handles_missing_git(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tunapi.utils.git.subprocess.run", _raise)
...
def test_git_stdout_returns_none_on_error(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tunapi.utils.git._run_git", _fake_run)
    assert git_stdout(["status"], cwd=Path("/repo")) is None


def test_git_ok_false_when_run_missing(monkeypatch) -> None:
    monkeypatch.setattr("tunapi.utils.git._run_git", lambda *_args, **_kwargs: None)
