...
from tunapi.runners.mock import Emit, Return, ScriptRunner, Wait
...
async def test_runner_contract_session_started_and_order() -> None:
    raw_completed: TunapiEvent = ActionEvent(
        engine=CODEX_ENGINE,
        action=Action(
            id="a-1",
            kind="command",
...
async def test_runner_releases_lock_when_consumer_closes() -> None:
    ...
    gen = cast(AsyncGenerator[TunapiEvent], runner.run("hello", None))
    try:
        while True:
            evt = await anext(gen)
