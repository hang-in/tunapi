...
- A Tunapi-owned **normalized event model** produced by runners and consumed by renderers/bridge
...
- **Runner**: Tunapi adapter that executes an engine process and yields **Tunapi events**.
- **Thread**: a single engine-side conversation, identified in Tunapi by a **ResumeToken**.
- **ResumeToken**: Tunapi-owned thread identifier `{ engine: EngineId, value: str }`.
...
- **TunapiEvent**: a normalized event emitted by a runner and consumed by renderers/bridge.
...
Tunapi MUST treat the runner as authoritative for:

- formatting a ResumeToken into a ResumeLine
- extracting a ResumeToken from message text

### 3.2 ResumeToken schema (Tunapi-owned)
...
Runners are responsible for emitting well-formed Tunapi events. Consumers (renderer/bridge) SHOULD assume validity and MAY fail fast on invariant violations.
...
* be deterministic functions/state machines over Tunapi events + internal renderer state
...
* Tunapi MUST support configuring a **default engine** used to start new threads (`resume=None`).
...
* If no engine subcommand is provided, Tunapi MUST run in **auto-router** mode:
  * new threads use the configured default engine
  * resumed threads are routed based on ResumeLine extraction (per §3.4)
* If an engine subcommand is provided, Tunapi MUST still use the auto-router, but it overrides the configured default engine for new threads.
...
Tunapi SHOULD keep the bot’s slash-command menu in sync at startup by calling
...
Tunapi MUST prevent multiple instances from racing `getUpdates` offsets for the same bot token.

### 10.1 Lock file location

The lock file MUST be stored at `<config_path>.lock`. For the default config path, this resolves to `~/.tunapi/tunapi.lock`.
...
- Initial minimal Tunapi specification (Telegram bridge + runner protocol + normalized events + resume support).
